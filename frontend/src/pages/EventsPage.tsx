...
import { Calendar, MapPin, Target, Bell, Users } from "lucide-react"
import { Card, CardContent, CardDescription, CardHeader, CardTitle } from "@/components/ui/card"
import { Button } from "@/components/ui/button"
import { Footer } from "@/components/Footer"
import { events, getEventTypeColor, type Event } from "@/data/events"

const badgeTheme = {
  past: "bg-teal-100  dark:bg-teal-900/30  text-teal-700  dark:text-teal-300",
  current: "bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-300",
  future: "bg-emerald-100 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-300",
} as const
...
function EventCard({ event, index }: { event: Event; index: number }) {
    ...
    >
      ...
      <CardHeader>
        <div className="flex items-start justify-between mb-2">
          <span className={`text-xs font-semibold px-2 py-1 rounded-full ${badgeTheme[event.status ?? "future"]}`}>
  ...
  if (isPastArtEvent) {
    return (
      <Link to={`/events/${event.id}`} className="flex flex-col no-underline">
        {cardContent}
      </Link>
    )
  }
...
export function EventsPage() {
  ...
  const [timeFilter, setTimeFilter] = useState<"allTime" | "past" | "current" | "future">("allTime")

  const filteredEvents = events
    .filter((e) =>
      filterType === "all" ? true : filterType === "all-literacy" ? e.type === "all" : e.type === filterType,
    )
    .filter((e) => (timeFilter === "allTime" ? true : e.status === timeFilter))
            ...
            <div className="px-4 md:px-6">
              <div className="max-w-7xl mx-auto text-center">
                <p className="text-muted-foreground">
                  {filterType !== "all" && (
                    <>
                      Showing <span className="font-semibold">Operation {filterType.toUpperCase()}</span> events:{" "}
                      {typeLabels[filterType === "all-literacy" ? "all" : filterType]}
                    </>
                  )}
                  {filterType !== "all" && timeFilter !== "allTime" && " • "}
                  {timeFilter !== "allTime" && (
                    <>
                      <span className="font-semibold">{badgeLabel[timeFilter as "past" | "current" | "future"]}</span>{" "}
                      timeline
                    </>
                  )}
                </p>
